---
title: "11. Pas algoritmen verantwoord toe"
summary: Ensure transparent, fair and responsible application of algorithms and AI systems.
relations:
  - data
  - veiligheid
  - privacy
  - gebruikersbehoefte
---


# 11. Apply algorithms responsibly

Algorithms and AI systems have an increasing impact on government decision-making. Responsible application requires transparency, clarity, honesty and human supervision to safeguard citizens' rights, prevent discrimination and maintain trust in digital public services.

## Why is it important?

- **Justice**: Prevent discrimination and unfair treatment of citizens
- **Transparency**: Clarify how decisions are made
- **Responsible**: Make sure responsibility is clearly invested
- **Confidence**: Maintain public trust in public services
- **Limitation regulation**: Comply with laws and regulations such as the AI Regulation
- **Quality**: Improve decision-making through good algorithm governance
- **Ethical action**: Guarantee fundamental rights in automated processes
- **Controllability**: Enable citizens to understand and challenge decisions

By applying algorithms responsibly, government organisations ensure that their digital systems function fairly, transparently and in line with public values, which is crucial to maintaining trust in an increasingly digitised government.

## How do you apply it?

<div class="direct-to-de-stroke">
<h3>Direct action</h3>

<div class="warning-banner" style="background-color: #fff3e0; padding: 0.5rem; border-left: 3px solid #ff9800; margin bottom: 0.8rem;">
<strong>Work in Progress:</strong> The functionality below is still under development.
</div>

<div class="action cards'>
<div class="action-card'>
<span class="wip-badge-badge-available">available</span>
<h4>Algorithm Frame</h4>
<p>Responsible algorithm management guidelines</p>
<a href="https://minbzk.github.io/Algorithmframe/" class="action-button" target="_white">Explore</a>
</div>
<div class="action-card'>
<span class="wip-badge-badge-available">available</span>
<h4>Delicate AI Regulation</h4>
<p>Define directly which rules apply</p>
<a href="https://ai-regulation-decision-help.apps.digilab.network/" class="action-button" target="_white">Start</a>
</div>
<div class="action-card'>
<span class="wip-badge-badge-available">available</span>
<h4>Algorithm Management Toolkit</h4>
<p>Register for risky algorithms</p>
<a href="https://amt.prd.apps.digilab.network/" class="action-button" target="_blank">Registration</a>
</div>
<div class="action-card'>
<span class="wip-badge-badge-available">available</span>
<h4>LLM Benchmark</h4>
<p>Benchmark for language models (LLMs)</p>
<a href="https://github.com/MinBZK/llm benchmark" class="action-button" target="_white">Explore</a>
</div>
<div class="action-card'>
<span class="wip-badge-badge-available">available</span>
<h4>Algorithm registry</h4>
<p>Register your (impactful) algorithm in the national register</p>
<a href="https://algorithms.government.en/en" class="action-button" target="_blank">Explore</a>
</div>
<div class="action-card'>
<span class="wip-badge-badge-available">available</span>
<h4>Unsupervised bias detection Tool</h4>
<p>A statistical tool identifying groups where an algorithm shows abnormal performance</p>
<a href="https://algorithmaudit.eu/technical-tools/bdt/" class="action-button" target="_white">Explore</a>
</div>
</div>
</div>

Implementation steps

- **Algorithm-impact assessment**: Perform a risk analysis with the Algorithm Framework
- **Design with values**: Integrate ethical values into the design
- **Data control**: Ensure quality and fair use of data
- **Documentation**: Document the algorithm, usage and decisions
- **Test on bias**: Check actively for prejudice in algorithms
- **Human surveillance**: Implement meaningful human control
- **Periodic evaluation**: Evaluate algorithms regularly
- **Transparency reporting**: Publish information about use in the Algorithm Register
- **Classification**: Determine the level of risk under the AI Regulation

### Challenges and solutions

- **Complexity vs. explainability**: Choose explanatory algorithms where possible
- **Detecting bias**: Implement tools and methods for detecting prejudice
- **Balan's automation and human judgment**: Design hybrid decision-making processes
- **Knowledge deficiency**: Invest in training and awareness
- **Existing systems**: Develop a plan to assess existing algorithms

## Algorithm registration and accounting

- Identify and classify risk algorithms
- Register high-risk algorithms in Algorithm
- Register algorithm in the __PRESERVE_0_
- Document purpose, operation and guarantees
- Create public algorithm descriptions for high-risk algorithms
- Evaluate and update information regularly

## Related Tools

- **[Rijksbrede Algoritmekader](https://minbzk.github.io/Algoritmekader/)***: Framework for risk analysis, responsible commitment and compliance
- **[Impact Assessment Mensenrechten en Algoritmen (IAMA)](https://www.rijksoverheid.nl/documenten/rapporten/2021/02/25/impact-assessment-mensenrechten-en-algoritmes)***: Methodology for assessing human rights impact
- **[Code Goed Digitaal Openbaar Bestuur](https://www.digitaleoverheid.nl/overzicht-van-alle-onderwerpen/nieuwe-technologieen-data-en-ethiek/publieke-waarden/code-goed-digitaal-openbaar-bestuur/)***: Digital governance guidelines

## Related guidelines

- [1. Stel gebruikersbehoeften vast](../gebruikersbehoeften/index.md)
- [6. Maak veilige systemen](../veiligheid/index.md)
- [7. Maak privacy integraal](../privacy/index.md)
- [10. Maak beter gebruik van data](../data/index.md)
