#!/usr/bin/env python3
"""
Validate richtlijn files in docs/richtlijnen/**/index.md
"""
...
def load_validation_rules():
    ...
    schema_path = Path("schemas/schema_richtlijn.json")
...
def find_richtlijn_files():
    """Find all index.md files in docs/richtlijnen subdirectories"""
    richtlijn_dir = Path("docs/richtlijnen")

    if not richtlijn_dir.exists():
        print("❌ docs/richtlijnen directory not found")
        return []

    files = []
    for subdir in richtlijn_dir.iterdir():
...
def main():
    ...
    print("🔍 Validating richtlijn files...")

    # Load validation rules
    rules = load_validation_rules()

    # Find files to validate
    files = find_richtlijn_files()

    if not files:
        print("❌ No richtlijn files found")
        return 1
